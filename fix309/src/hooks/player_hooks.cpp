...
struct PreNotifyStaminaSlot {
    std::atomic<uintptr_t> entry{0};
    std::atomic<int64_t> value{0};
    std::atomic<DWORD> tick{0};
};

PreNotifyStaminaSlot g_prenotify_stamina_slots[8];
...
PreNotifyStaminaSlot* FindPreNotifyStaminaSlot(const uintptr_t entry) {
    if (entry < kMinimumPointerAddress) {
        return nullptr;
    }

    for (auto& slot : g_prenotify_stamina_slots) {
        if (slot.entry.load(std::memory_order_acquire) == entry) {
            return &slot;
        }
    }
    return nullptr;
...
PreNotifyStaminaSlot* AcquirePreNotifyStaminaSlot(const uintptr_t entry) {
    if (entry < kMinimumPointerAddress) {
        return nullptr;
    }

    if (auto* const existing = FindPreNotifyStaminaSlot(entry)) {
        return existing;
    }

    for (auto& slot : g_prenotify_stamina_slots) {
        uintptr_t empty = 0;
        if (slot.entry.compare_exchange_strong(empty, entry, std::memory_order_acq_rel)) {
    ...
    auto* oldest = &g_prenotify_stamina_slots[0];
    DWORD oldest_tick = 0xFFFFFFFFu;
    for (auto& slot : g_prenotify_stamina_slots) {
        const DWORD tick = slot.tick.load(std::memory_order_acquire);
        if (tick < oldest_tick) {
            oldest_tick = tick;
            oldest = &slot;
        }
    ...
    oldest->entry.store(entry, std::memory_order_release);
    oldest->value.store(0, std::memory_order_release);
    oldest->tick.store(0, std::memory_order_release);
    return oldest;
}

bool LoadOrInitPreNotifyStaminaCache(const uintptr_t entry,
                                     const int64_t observed_value,
                                     const int64_t max_value,
                                     int64_t* const cached_value) {
    if (cached_value == nullptr || max_value <= 0) {
        return false;
    }

    auto* const slot = AcquirePreNotifyStaminaSlot(entry);
    if (slot == nullptr) {
        return false;
    }

    int64_t value = slot->value.load(std::memory_order_acquire);
    if (value < 0 || value > max_value) {
        value = std::clamp(observed_value, int64_t{0}, max_value);
        slot->value.store(value, std::memory_order_release);
        slot->tick.store(GetTickCount(), std::memory_order_release);
        return false;
    }
...
void StorePreNotifyStaminaCache(const uintptr_t entry, const int64_t value) {
    auto* const slot = AcquirePreNotifyStaminaSlot(entry);
    if (slot == nullptr) {
        return;
    }
...
bool TryAdjustTrackedStaminaPreNotify(const uintptr_t entry,
    ...
    if (!LoadOrInitPreNotifyStaminaCache(entry, observed_value, max_value, &last_value)) {
        return false;
    }
    ...
    if (delta > 0 &&
        ...
        StorePreNotifyStaminaCache(entry, observed_value);
    ...
    if (delta > 0 && observed_value == max_value && max_value > 500000 && WasRecentPlayerStaminaConsumption(entry, 900)) {
        const int64_t final_value = std::clamp(last_value, int64_t{0}, max_value);
        StorePreNotifyStaminaCache(entry, final_value);
...
void StatWriteCallback(SafetyHookContext& ctx) {
    ...
    __try {
        ...
        if (entry_is_trusted_mount_stamina) {
            ...
            const auto& config = GetConfig();
            const bool mount_stamina_lock_enabled =
                config.mount.enabled &&
                (config.mount.lock_stamina ||
                 (config.mount.special_enabled && config.mount.lock_special_stamina));
            if (mount_stamina_lock_enabled && ctx.rbx != 0) {
                ctx.rbx = 0;
            }
            if (log_sample) {
                Log("hooks: stamina-ab00 routed trusted mount stamina entry=0x%p delta=%lld player_stamina=0x%p mount_stamina=0x%p note=d90-mount-cost-zero",
                    reinterpret_cast<void*>(entry),
                    static_cast<long long>(ctx.rbx),
                    reinterpret_cast<void*>(tracked_player_stamina),
        ...
        if (entry == tracked_player_stamina) {
            ...
            const bool visual_write = TryPublishTrackedStaminaAb00VisualCost(entry, player_stamina_delta, nullptr);
            ...
            if (log_sample || trace_sample) {
                ...
                Log("hooks: stamina-ab00 player stamina passthrough entry=0x%p delta=%lld owner=0x%p status=%u rdi=0x%p r14=0x%p current=%lld max=%lld read_ok=%d visual_write=%d note=d90-d75-no-ab00-owner-promotion",
                    reinterpret_cast<void*>(entry),
