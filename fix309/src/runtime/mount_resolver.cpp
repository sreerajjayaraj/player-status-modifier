...
bool LooksLikePlayerCombatProfile(const ActorResolveSnapshot& snapshot,
                                  const int64_t health_max,
                                  const int64_t stamina_max) {
    if (!IsInClosedRange(health_max, 100000, 2500000) ||
        !IsInClosedRange(stamina_max, 400000, 700000) ||
        snapshot.health_entry + kStaminaEntryOffsetFromHealth != snapshot.stamina_entry) {
        return false;
    }

    const uintptr_t primary_spirit = snapshot.health_entry + kSpiritEntryOffsetFromHealth;
    if (primary_spirit < kMinimumPointerAddress) {
        return false;
    }
    ...
    __try {
        spirit_type = *reinterpret_cast<const int32_t*>(primary_spirit);
        spirit_max = *reinterpret_cast<const int64_t*>(primary_spirit + 0x18);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        return false;
    }

    return IsPrimarySpiritStatId(spirit_type) && IsInClosedRange(spirit_max, 100000, 220000);
}
...
bool IsSupportedMountProfile(const ActorResolveSnapshot& snapshot) {
    ...
    if (LooksLikePlayerCombatProfile(snapshot, health_max, stamina_max)) {
        return false;
    }
