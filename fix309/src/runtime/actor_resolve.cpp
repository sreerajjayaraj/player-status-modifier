...
constexpr int64_t kPlayerCombatStaminaResolveMaxMax = 700000;
...
void ClearTrustedMountOverlapForPlayerPromotion(const ActorResolveSnapshot& snapshot,
                                                const char* const source,
                                                const char* const reason,
                                                const uintptr_t entry) {
    if (!OverlapsTrustedMountResolve(snapshot)) {
        return;
    }
    ...
    if (current < 96) {
        Log("runtime: cleared stale trusted mount overlap for player promotion source=%s reason=%s entry=0x%p stale_mount_health=0x%p stale_mount_stamina=0x%p new_player_health=0x%p new_player_stamina=0x%p",
...
bool TryResolvePlayerCombatBlockFromHealth(const uintptr_t health_entry,
    ...
    const uintptr_t spirit_entry = health_entry + kSpiritEntryOffsetFromHealth;
    int64_t spirit_max = 0;
    int32_t spirit_type = -1;
    if (!TryReadStatTypeValue(spirit_entry, &spirit_type) ||
        !IsPrimarySpiritStatId(spirit_type) ||
        !TryGetStatEntryMaxValue(spirit_entry, kSpiritId, &spirit_max) ||
        !IsPlayerCombatSpiritMaxValue(spirit_max)) {
        return false;
    }
...
bool TryResolvePlayerCombatBlockFromSpiritPair(const uintptr_t spirit_entry,
                                               ActorResolveSnapshot* const resolved) {
    if (resolved == nullptr || spirit_entry < kMinimumPointerAddress) {
        return false;
    }
    ...
    if (!TryReadStatTypeValue(spirit_entry, &spirit_type) ||
        ...
        !TryGetStatEntryMaxValue(spirit_entry, kSpiritId, &spirit_max) ||
        !IsPlayerCombatSpiritMaxValue(spirit_max) ||
        spirit_entry <= kSpiritEntryOffsetFromHealth) {
        return false;
    }

    const uintptr_t candidate_health = spirit_entry - kSpiritEntryOffsetFromHealth;
    int64_t health_max = 0;
    if (!TryGetStatEntryMaxValue(candidate_health, kHealthId, &health_max) ||
        !IsPlayerCombatHealthMaxValue(health_max)) {
        return false;
    }
    ...
    for (const uintptr_t stamina_offset : stamina_offsets) {
        const uintptr_t candidate_stamina = candidate_health + stamina_offset;
        int64_t stamina_max = 0;
        if (!TryGetStaminaEntryMaxValue(candidate_stamina, &stamina_max) ||
            !IsPlayerCombatStaminaMaxValue(stamina_max) ||
            !HasExpectedHealthStaminaLayout(candidate_health, candidate_stamina)) {
            continue;
        }

        *resolved = g_player_resolve;
        resolved->health_entry = candidate_health;
        resolved->stamina_entry = candidate_stamina;
        resolved->spirit_entry = spirit_entry;
        return true;
    }
...
bool TryPromotePlayerCombatResourcesFromStaminaEntry(const uintptr_t stamina_entry, const char* const reason) {
    ...
    ClearTrustedMountOverlapForPlayerPromotion(resolved, "stamina", reason, stamina_entry);
...
bool TryPromotePlayerCombatResourcesFromHealthEntry(const uintptr_t health_entry, const char* const reason) {
    ...
    ClearTrustedMountOverlapForPlayerPromotion(resolved, "health", reason, health_entry);
...
bool TryPromotePlayerCombatResourcesFromSpiritEntry(const uintptr_t spirit_entry, const char* const reason) {
    ...
    ActorResolveSnapshot resolved{};
    bool strict_resolve = false;
    if (spirit_entry > kSpiritEntryOffsetFromHealth &&
        TryResolvePlayerCombatBlockFromHealth(spirit_entry - kSpiritEntryOffsetFromHealth, &resolved) &&
        resolved.spirit_entry == spirit_entry) {
        strict_resolve = true;
    } else {
        resolved = {};
    ...
    if (!strict_resolve &&
        !TryResolvePlayerCombatBlockFromSpiritPair(spirit_entry, &resolved)) {
        return false;
    }

    ClearTrustedMountOverlapForPlayerPromotion(resolved, "spirit", reason, spirit_entry);
    ...
    const auto current = g_discovery_logs.fetch_add(1, std::memory_order_acq_rel);
    if (current < 32) {
        Log("runtime: promoted active player resources from spirit reason=%s mode=%s health=0x%p stamina=0x%p spirit=0x%p previous_health=0x%p previous_stamina=0x%p previous_spirit=0x%p",
            reason != nullptr ? reason : "unknown",
            strict_resolve ? "strict" : "adjacent-pair",
            reinterpret_cast<void*>(g_player_resolve.health_entry),
            reinterpret_cast<void*>(g_player_resolve.stamina_entry),
            reinterpret_cast<void*>(g_player_resolve.spirit_entry),
            reinterpret_cast<void*>(existing.health_entry),
