...
#ifndef PLAYER_STATUS_MODIFIER_PRIMARY_CONFIG_NAME
#define PLAYER_STATUS_MODIFIER_PRIMARY_CONFIG_NAME L"player-status-modifier.ini"
#endif

#ifndef PLAYER_STATUS_MODIFIER_DEFAULT_CONFIG_NAME
#define PLAYER_STATUS_MODIFIER_DEFAULT_CONFIG_NAME L"player-status-modifier.default.ini"
#endif

#ifndef PLAYER_STATUS_MODIFIER_LAYER_PREFIX
#define PLAYER_STATUS_MODIFIER_LAYER_PREFIX L"player-status-modifier."
#endif
...
bool IsPrimaryConfigName(const std::filesystem::path& path) {
    return _wcsicmp(path.filename().c_str(), PLAYER_STATUS_MODIFIER_PRIMARY_CONFIG_NAME) == 0;
}

bool IsDefaultConfigName(const std::filesystem::path& path) {
    return _wcsicmp(path.filename().c_str(), PLAYER_STATUS_MODIFIER_DEFAULT_CONFIG_NAME) == 0;
}

bool IsLayerConfigName(const std::filesystem::path& path) {
    const auto file_name = path.filename().wstring();
    constexpr wchar_t kPrefix[] = PLAYER_STATUS_MODIFIER_LAYER_PREFIX;
...
void ApplyConfigLayer(const std::wstring& path, ModConfig* const next) {
    ...
    ReadBoolIfPresent(L"Advanced", L"SwapPlayerStaminaSpirit", &next->advanced.swap_player_stamina_spirit, path);
    ReadBoolIfPresent(L"Advanced",
                      L"RedirectLargeStaminaCostsToSpirit",
                      &next->advanced.redirect_large_stamina_costs_to_spirit,
                      path);
    ReadBoolIfPresent(L"Advanced",
                      L"SuppressRedirectedStaminaCost",
                      &next->advanced.suppress_redirected_stamina_cost,
                      path);
    if (HasIniKey(L"Advanced", L"SpiritStaminaRedirectMinCost", path)) {
        DWORD min_cost = static_cast<DWORD>(next->advanced.spirit_stamina_redirect_min_cost);
        ReadDwordIfPresent(L"Advanced", L"SpiritStaminaRedirectMinCost", &min_cost, path);
        next->advanced.spirit_stamina_redirect_min_cost = static_cast<int64_t>(min_cost);
    }
    ReadDoubleIfPresent(L"Advanced",
                        L"SpiritStaminaRedirectScale",
                        &next->advanced.spirit_stamina_redirect_scale,
                        path);
    ReadDwordIfPresent(L"Advanced",
                       L"FocusRecoveryWindowMs",
                       &next->advanced.focus_recovery_window_ms,
                       path);
    ...
    ReadBoolIfPresent(L"Mount", L"LockSpiritStamina", &next->mount.lock_spirit_stamina, path);
    ...
    ReadBoolAliasIfPresent(L"SpecialMount", L"Enabled", L"Enable", &next->mount.special_enabled, path);
    ReadBoolIfPresent(L"SpecialMount", L"LockHealth", &next->mount.lock_special_health, path);
    ReadBoolIfPresent(L"SpecialMount", L"LockStamina", &next->mount.lock_special_stamina, path);
    ReadBoolIfPresent(L"SpecialMount", L"LockSpiritStamina", &next->mount.lock_special_spirit_stamina, path);
    ReadBoolIfPresent(L"SpecialMount", L"LockHealthCandidates", &next->mount.lock_special_health_candidates, path);
    if (HasIniKey(L"SpecialMount", L"LockValue", path)) {
        DWORD lock_value = static_cast<DWORD>(next->mount.special_lock_value);
        ReadDwordIfPresent(L"SpecialMount", L"LockValue", &lock_value, path);
        next->mount.special_lock_value = static_cast<int64_t>(lock_value);
    }
    ...
    const auto read_stat_section = [&](const wchar_t* const section, StatConfig* const stat) {
        ...
        const bool has_multiplier =
            HasIniKey(section, L"ConsumptionMultiplier", path) ||
            HasIniKey(section, L"HealMultiplier", path) ||
            HasIniKey(section, L"RecoveryLockoutMs", path);
        ...
        ReadDwordIfPresent(section, L"RecoveryLockoutMs", &stat->recovery_lockout_ms, path);
        ...
        if (!has_explicit_enable &&
            has_multiplier &&
            (stat->consumption_multiplier != 1.0 ||
             stat->heal_multiplier != 1.0 ||
             stat->recovery_lockout_ms != 0)) {
            stat->enabled = true;
        }
    };
...
void SanitizeConfig(ModConfig* const next) {
    ...
    if (next->advanced.spirit_stamina_redirect_min_cost <= 0) {
        next->advanced.spirit_stamina_redirect_min_cost = 50000;
    }

    next->advanced.spirit_stamina_redirect_scale =
        ClampDouble(next->advanced.spirit_stamina_redirect_scale, 0.01, 10.0, 1.0);
    if (next->advanced.focus_recovery_window_ms < 1000) {
        next->advanced.focus_recovery_window_ms = 1000;
    } else if (next->advanced.focus_recovery_window_ms > 30000) {
        next->advanced.focus_recovery_window_ms = 30000;
    }
    ...
    if (next->mount.special_lock_value <= 0) {
        next->mount.special_lock_value = 9999999;
    }

    if (!next->health.enabled) {
        next->health.consumption_multiplier = 1.0;
        next->health.heal_multiplier = 1.0;
        next->health.recovery_lockout_ms = 0;
    }

    if (!next->stamina.enabled) {
        next->stamina.consumption_multiplier = 1.0;
        next->stamina.heal_multiplier = 1.0;
        next->stamina.recovery_lockout_ms = 0;
    }

    if (!next->spirit.enabled) {
        next->spirit.consumption_multiplier = 1.0;
        next->spirit.heal_multiplier = 1.0;
        next->spirit.recovery_lockout_ms = 0;
    } else if (next->spirit.recovery_lockout_ms > 10000) {
        next->spirit.recovery_lockout_ms = 10000;
    }
}

#ifdef PLAYER_STATUS_MODIFIER_ECONOMY_ONLY
void SanitizeEconomyOnlyConfig(ModConfig* const next) {
    if (next == nullptr) {
        return;
    }

    next->damage.outgoing.enabled = false;
    next->damage.outgoing.stat_write_fallback = false;
    next->damage.outgoing.multiplier = 1.0;

    next->damage.incoming.stat_write_fallback = false;
...
std::vector<std::wstring> GetConfigMergePaths(const std::wstring& config_path) {
    ...
    if (!directory.empty() && std::filesystem::is_directory(directory, ec)) {
        const auto default_path = directory / PLAYER_STATUS_MODIFIER_DEFAULT_CONFIG_NAME;
...
bool ReadConfigSnapshot(const std::wstring& config_path, ModConfig* const config) {
    ...
    SanitizeConfig(&next);
#ifdef PLAYER_STATUS_MODIFIER_ECONOMY_ONLY
    SanitizeEconomyOnlyConfig(&next);
#endif
    *config = next;
    return true;
}
