...
bool TryScalePlayerDamage(const uintptr_t target,
    ...
    bool target_is_playable_resource_owner =
        target >= kMinimumPointerAddress &&
        refreshed_player_target_owner >= kMinimumPointerAddress &&
        target == refreshed_player_target_owner;
    if (!target_is_playable_resource_owner && target >= kMinimumPointerAddress) {
        ActorResolveSnapshot resolved_target{};
        ActorResolveSnapshot playable_target{};
        if (TryResolveActorResolveFromRoot(target, &resolved_target) &&
            resolved_target.health_entry >= kMinimumPointerAddress &&
            TryResolvePlayerCombatResourcesFromHealthEntry(resolved_target.health_entry, &playable_target)) {
            target_is_playable_resource_owner = true;
        }
    }

    if (target >= kMinimumPointerAddress &&
        target_is_playable_resource_owner &&
        *value > 0) {
    ...
    if (target >= kMinimumPointerAddress && target_is_playable_resource_owner) {
        ...
        if (ShouldLogDamageRuntime()) {
            Log("runtime: scaled damage direction=%s target=0x%p sourceCtx=0x%p old_delta=%lld final=%lld incoming_enabled=%d incoming_multiplier=%.3f effective_multiplier=%.3f",
                "incoming-damage",
                reinterpret_cast<void*>(target),
                reinterpret_cast<void*>(source_context),
                static_cast<long long>(original_delta),
