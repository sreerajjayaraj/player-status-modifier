...
bool LooksLikePrimaryItemGainCall(const SafetyHookContext& ctx) {
    uintptr_t return_address = 0;
    uintptr_t stack_28 = 0;
    uintptr_t tramp_20 = 0;
    if (!TryReadValue(ctx.rsp + 0x00, &return_address) ||
        !TryReadValue(ctx.rsp + 0x28, &stack_28) ||
        !TryReadValue(ctx.trampoline_rsp + 0x20, &tramp_20)) {
        return false;
    }

    return return_address >= kMinimumPointerAddress &&
           stack_28 >= kMinimumPointerAddress &&
           tramp_20 >= kMinimumPointerAddress;
}
...
void ItemGainCallback(SafetyHookContext& ctx) {
    ...
    __try {
        ...
        if (!LooksLikePrimaryItemGainCall(ctx)) {
            if (log_sample || count < 10 || affinity_diagnostic_sample || aggressive_sample) {
                Log("hooks: item-gain skipped shadow call amount=%lld target=0x%p",
                    static_cast<long long>(adjusted_amount),
                    reinterpret_cast<void*>(target));
            }
            return;
