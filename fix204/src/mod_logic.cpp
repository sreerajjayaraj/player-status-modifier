...
void UpdateTrackedMountFromStaminaContext(const uintptr_t stamina_entry, const uintptr_t context_root) {
    ...
    if (stamina_entry == GetTrackedPlayerStaminaEntry()) {
        return;
    }
    ...
    if (max_value >= 400000 &&
        ...
        if (TryPromotePlayerCombatResourcesFromStaminaEntry(stamina_entry, "save-load loose player-shaped stamina")) {
            const auto current = g_loose_mount_stamina_logs.fetch_add(1, std::memory_order_acq_rel);
            if (current < 32) {
                Log("runtime: promoted loose player-shaped stamina during mount update entry=0x%p spirit=0x%p stamina_max=%lld spirit_max=%lld context=0x%p",
                    reinterpret_cast<void*>(stamina_entry),
                    reinterpret_cast<void*>(adjacent_spirit),
                    static_cast<long long>(max_value),
                    static_cast<long long>(adjacent_spirit_max),
        ...
        const auto current = g_loose_mount_stamina_logs.fetch_add(1, std::memory_order_acq_rel);
        if (current < 32) {
            Log("runtime: rejected loose mount stamina as player-shaped combat resource entry=0x%p spirit=0x%p stamina_max=%lld spirit_max=%lld context=0x%p",
                reinterpret_cast<void*>(stamina_entry),
                reinterpret_cast<void*>(adjacent_spirit),
                static_cast<long long>(max_value),
                static_cast<long long>(adjacent_spirit_max),
...
void UpdateTrackedPlayerStatusComponent(const uintptr_t actor, const uintptr_t component) {
    ...
    if (!TryResolveActorResolveFromMarker(component, &resolved, actor)) {
        ...
        bool should_refresh_mount = false;
        {
            ...
            const auto current = g_actor_resolve_logs.fetch_add(1, std::memory_order_acq_rel);
            if (current < 24) {
                Log("runtime: tracked player actor/root via pointer fallback actor=0x%p status_marker=0x%p stat_root=0x%p preserved_health=0x%p preserved_stamina=0x%p preserved_spirit=0x%p",
...
void UpdateTrackedPlayerResourceOwner(const uintptr_t owner_root, const char* const reason) {
    ...
    if (old_root == owner_root && old_damage_target == owner_root) {
        return;
    }

    g_player_resolve.root = owner_root;
    g_player_resolve.damage_target = owner_root;

    if (g_mount_resolve.root == owner_root ||
    ...
    const auto current = g_actor_resolve_logs.fetch_add(1, std::memory_order_acq_rel);
    if (current < 48) {
        Log("runtime: updated player damage owner from resource write reason=%s old_root=0x%p old_damage_target=0x%p new_root=0x%p health=0x%p stamina=0x%p spirit=0x%p",
