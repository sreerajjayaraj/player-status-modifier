...
void StatWriteCallback(SafetyHookContext& ctx) {
    ...
    __try {
        ...
        // Exact tracked entries can see an r8/rsi mirror that should not be
        // rewritten. Detached post-rematch player resources, however, still
        // use rbx for the actual stat value even when r8/rsi carries another
        // changing counter. Prefer rbx for those learned candidates.
        const int64_t observed_value = static_cast<int64_t>(
            exact_tracked_player_entry && value_in_r8_rsi ? ctx.r8 : ctx.rbx);
        const bool log_sample = exact_tracked_player_entry && ShouldLogSample(g_stat_write_samples, 24);
        if (log_sample) {
            Log("hooks: stat-write callback entry=0x%p value=%lld source=%s rbx=%lld rsi=%lld r8=%lld tracked_player_health=0x%p tracked_player_stamina=0x%p tracked_player_spirit=0x%p matched=%d rip=0x%p",
                reinterpret_cast<void*>(ctx.rdi),
                static_cast<long long>(observed_value),
                exact_tracked_player_entry && value_in_r8_rsi ? "r8/rsi" : "rbx",
        ...
        if (exact_tracked_player_entry && value_in_r8_rsi) {
            return;
        }
