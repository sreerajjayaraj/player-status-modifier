...
#ifndef PLAYER_STATUS_MODIFIER_BUILD_ID
#define PLAYER_STATUS_MODIFIER_BUILD_ID "safe-rva-fixed-fix61-no-stamina-spirit-swap"
#endif

#ifndef PLAYER_STATUS_MODIFIER_CONFIG_FILE_NAME
#define PLAYER_STATUS_MODIFIER_CONFIG_FILE_NAME L"player-status-modifier.ini"
#endif

#ifndef PLAYER_STATUS_MODIFIER_LOG_FILE_NAME
#define PLAYER_STATUS_MODIFIER_LOG_FILE_NAME L"player-status-modifier.log"
#endif

constexpr char kModSourceBuildId[] = PLAYER_STATUS_MODIFIER_BUILD_ID;
...
void LogEffectiveConfig(const ModConfig& config) {
    ...
    Log("config: Advanced SwapPlayerStaminaSpirit=%d",
        config.advanced.swap_player_stamina_spirit ? 1 : 0);
    Log("config: Advanced RedirectLargeStaminaCostsToSpirit=%d SuppressRedirectedStaminaCost=%d MinCost=%lld Scale=%.3f",
        config.advanced.redirect_large_stamina_costs_to_spirit ? 1 : 0,
        config.advanced.suppress_redirected_stamina_cost ? 1 : 0,
    ...
    Log("config: Resistance Enabled=%d Fire=%.3f Ice=%.3f Electricity=%.3f EffectiveIncomingMultiplier=%.3f",
        ...
        config.resistance.enabled
            ? (1.0 - std::max(std::max(config.resistance.fire_resistance, config.resistance.ice_resistance),
                              config.resistance.electricity_resistance))
            : 1.0);
    Log("config: Mount Enabled=%d LockHealth=%d LockStamina=%d LockValue=%lld SpecialEnabled=%d SpecialLockHealth=%d SpecialLockStamina=%d SpecialLockValue=%lld",
        ...
        config.mount.special_enabled ? 1 : 0,
        config.mount.lock_special_health ? 1 : 0,
        config.mount.lock_special_stamina ? 1 : 0,
        static_cast<long long>(config.mount.special_lock_value));
...
DWORD InitializeModUnchecked() {
    ...
    const auto config_path = GetSiblingPath(PLAYER_STATUS_MODIFIER_CONFIG_FILE_NAME);
    const auto log_path = GetSiblingPath(PLAYER_STATUS_MODIFIER_LOG_FILE_NAME);
    ...
    Log("dllmain: source build = %s compiled=%s %s stamina-id=%d spirit-id=%d legacy-spirit-id=%d stamina-offset=0x%llX legacy-offset=0x%llX spirit-offset=0x%llX",
        kModSourceBuildId,
        __DATE__,
        __TIME__,
        kStaminaId,
...
#ifndef PLAYER_STATUS_MODIFIER_ECONOMY_ONLY
    ...
    if (!StartSpiritWatcher()) {
        Log("dllmain: spirit watcher failed to start");
        CleanupAfterInitializationFailure();
        return 0;
    }
#endif
