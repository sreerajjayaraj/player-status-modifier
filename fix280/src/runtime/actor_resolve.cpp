...
bool TryResolvePlayerCombatBlockFromSpiritPair(const uintptr_t spirit_entry,
    ...
    int64_t spirit_max = 0;
    if (!TryGetStatEntryMaxValue(spirit_entry, kSpiritId, &spirit_max) ||
    ...
    for (const uintptr_t spirit_offset : spirit_offsets) {
        if (spirit_entry <= spirit_offset) {
            continue;
        }

        const uintptr_t candidate_health = spirit_entry - spirit_offset;
...
bool TryPromotePlayerCombatResourcesFromStaminaEntry(const uintptr_t stamina_entry, const char* const reason) {
    ...
    if (OverlapsTrustedMountResolve(resolved)) {
        ...
        if (current < 96) {
            Log("runtime: rejected player promotion from stamina reason=%s entry=0x%p note=trusted-mount-overlap",
...
bool TryPromotePlayerCombatResourcesFromHealthEntry(const uintptr_t health_entry, const char* const reason) {
    ...
    if (OverlapsTrustedMountResolve(resolved)) {
        const auto current = g_actor_resolve_logs.fetch_add(1, std::memory_order_acq_rel);
        if (current < 96) {
            Log("runtime: rejected player promotion from health reason=%s entry=0x%p note=trusted-mount-overlap",
                reason != nullptr ? reason : "unknown",
...
bool TryPromotePlayerCombatResourcesFromSpiritEntry(const uintptr_t spirit_entry, const char* const reason) {
    ...
    if (OverlapsTrustedMountResolve(resolved)) {
        const auto current = g_actor_resolve_logs.fetch_add(1, std::memory_order_acq_rel);
        if (current < 96) {
            Log("runtime: rejected player promotion from spirit reason=%s entry=0x%p note=trusted-mount-overlap",
                reason != nullptr ? reason : "unknown",
