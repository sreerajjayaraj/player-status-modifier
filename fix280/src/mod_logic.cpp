...
void UpdateTrackedPlayerResourceOwner(const uintptr_t owner_root, const char* const reason) {
    ...
    const uintptr_t new_root = same_resource_profile && owner_resolved.root >= kMinimumPointerAddress
        ? owner_resolved.root
        : owner_root;
    if (old_root == new_root && old_damage_target == new_root) {
        return;
    }

    g_player_resolve.root = new_root;
    g_player_resolve.damage_target = new_root;
    if (same_resource_profile) {
        if (g_player_resolve.actor < kMinimumPointerAddress) {
            g_player_resolve.actor = owner_resolved.actor;
        }
        if (g_player_resolve.marker < kMinimumPointerAddress) {
            g_player_resolve.marker = owner_resolved.marker;
        }
    }
