...
std::array<FriendlyDataScratch, 64> g_friendly_scratch_records{};
std::atomic<std::size_t> g_friendly_scratch_index{0};
...
bool TryPrepareScaledFriendlyDataCopy(const char* const path_name,
    ...
    const auto scratch_index = g_friendly_scratch_index.fetch_add(1, std::memory_order_acq_rel);
    auto& scratch = g_friendly_scratch_records[scratch_index % g_friendly_scratch_records.size()];
...
bool ResolveKnownCallsiteTarget(const char* const name,
                                const KnownCallsite* const callsites,
                                const std::size_t callsite_count,
                                uintptr_t* const target) {
    if (target == nullptr || callsites == nullptr || callsite_count == 0) {
        return false;
    }

    for (std::size_t i = 0; i < callsite_count; ++i) {
        if (ResolveHardcodedTarget(name, callsites[i].rva, callsites[i].expected, target)) {
            return true;
        }
    }
...
void InstallAffinityItemEventProbeHooks() {
    static constexpr KnownCallsite kGiveEvents[] = {
        {0x0138B64B, {0xE8, 0xB0, 0xD4, 0x96, 0x00}},  // CrimsonDesert.exe 1.0.0.1235
        {0x01377454, {0xE8, 0x17, 0xDD, 0x95, 0x00}},  // CrimsonDesert.exe 1.05.01
    };
    static constexpr KnownCallsite kTakeEvents[] = {
        {0x0138B742, {0xE8, 0xB9, 0xD3, 0x96, 0x00}},  // CrimsonDesert.exe 1.0.0.1235
        {0x0137754B, {0xE8, 0x20, 0xDC, 0x95, 0x00}},  // CrimsonDesert.exe 1.05.01
    };

    uintptr_t give_target = 0;
    uintptr_t take_target = 0;
    const bool give_found = ResolveKnownCallsiteTarget("affinity-item-give-event",
                                                       kGiveEvents,
                                                       std::size(kGiveEvents),
                                                       &give_target);
    const bool take_found = ResolveKnownCallsiteTarget("affinity-item-take-event",
                                                       kTakeEvents,
                                                       std::size(kTakeEvents),
                                                       &take_target);
