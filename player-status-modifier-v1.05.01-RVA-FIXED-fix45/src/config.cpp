...
void ApplyConfigLayer(const std::wstring& path, ModConfig* const next) {
    ...
    ReadBoolAliasIfPresent(L"Health", L"Enabled", L"Enable", &next->health.enabled, path);
    ReadDoubleIfPresent(L"Health", L"ConsumptionMultiplier", &next->health.consumption_multiplier, path);
    ReadDoubleIfPresent(L"Health", L"HealMultiplier", &next->health.heal_multiplier, path);

    ReadBoolAliasIfPresent(L"Stamina", L"Enabled", L"Enable", &next->stamina.enabled, path);
    ReadDoubleIfPresent(L"Stamina", L"ConsumptionMultiplier", &next->stamina.consumption_multiplier, path);
    ReadDoubleIfPresent(L"Stamina", L"HealMultiplier", &next->stamina.heal_multiplier, path);

    ReadBoolAliasIfPresent(L"Spirit", L"Enabled", L"Enable", &next->spirit.enabled, path);
    ReadDoubleIfPresent(L"Spirit", L"ConsumptionMultiplier", &next->spirit.consumption_multiplier, path);
    ReadDoubleIfPresent(L"Spirit", L"HealMultiplier", &next->spirit.heal_multiplier, path);
}
...
std::vector<std::wstring> GetConfigMergePaths(const std::wstring& config_path) {
    ...
    if (!directory.empty() && std::filesystem::is_directory(directory, ec)) {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(directory, ec)) {
            ...
            if (!IsRegularFile(path) || IsPrimaryConfigName(path) || !IsLayerConfigName(path)) {
                continue;
            }
        ...
        std::sort(layers.begin(), layers.end(), [](const auto& left, const auto& right) {
            const bool left_default = _wcsicmp(left.filename().c_str(), L"player-status-modifier.default.ini") == 0;
            const bool right_default = _wcsicmp(right.filename().c_str(), L"player-status-modifier.default.ini") == 0;
