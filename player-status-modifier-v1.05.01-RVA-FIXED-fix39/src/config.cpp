...
bool ReadConfigSnapshot(const std::wstring& config_path, ModConfig* const config) {
    ...
    next.damage.outgoing.stat_write_fallback =
        ReadBool(L"OutgoingDamage", L"UseStatWriteFallback", next.damage.outgoing.stat_write_fallback, config_path);
    next.damage.outgoing.multiplier =
        ...
        next.damage.incoming.stat_write_fallback =
            ReadBool(L"IncomingDamage", L"UseStatWriteFallback", next.damage.incoming.stat_write_fallback, config_path);
    ...
    next.health.enabled = ReadBoolAlias(L"Health", L"Enabled", L"Enable", next.health.enabled, config_path);
    ...
    next.stamina.enabled = ReadBoolAlias(L"Stamina", L"Enabled", L"Enable", next.stamina.enabled, config_path);
    ...
    next.spirit.enabled = ReadBoolAlias(L"Spirit", L"Enabled", L"Enable", next.spirit.enabled, config_path);
