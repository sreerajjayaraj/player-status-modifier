...
bool AreAffinityHooksInstalled(const ModConfig& config) {
    return !ShouldInstallAffinityHook(config) ||
           (g_affinity_hook && g_affinity_current_hook) ||
           (g_affinity_vary_hook && g_affinity_vary_logout_hook);
}
...
void LogHookLoadout(const ModConfig& config) {
    Log("hooks: loadout player-pointer=%d shared-stats=%d stat-write=%d spirit-delta=%d stamina-ab00=%d damage=%d items=%d affinity-prepare=%d affinity-current=%d affinity-vary=%d affinity-logout=%d durability=%d dragon-village=%d dragon-flying=%d dragon-roof=%d position=%d",
